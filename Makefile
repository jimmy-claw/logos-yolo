...
build-module: setup-nix-merged
	...
	cd $(BUILD_MODULE) && cmake .. $(CMAKE_FLAGS) \
		...
		&& cmake --build . --target yolo_plugin -j$$(nproc)
	@echo "yolo_plugin ready at: $(BUILD_MODULE)/yolo_plugin.so"

## Install yolo to logos-app modules dir
install-module: build-module
	mkdir -p ~/.local/share/Logos/LogosAppNix/modules/yolo
	cp $(BUILD_MODULE)/yolo_plugin.so ~/.local/share/Logos/LogosAppNix/modules/yolo/
	cp metadata.json ~/.local/share/Logos/LogosAppNix/modules/yolo/manifest.json
	@echo "yolo installed to ~/.local/share/Logos/LogosAppNix/modules/yolo/"
...
build-ui-plugin: setup-nix-merged
	...
	cd $(BUILD_UI_PLUGIN) && cmake .. $(CMAKE_FLAGS) \
		...
		-DLOGOS_LIBLOGOS_ROOT=/tmp/logos-liblogos-merged \
		$(if $(NIX_QTBASE),-DCMAKE_PREFIX_PATH="$(NIX_QT_PREFIX)" -DQT_ADDITIONAL_PACKAGES_PREFIX_PATH="$(NIX_QTDECL)$$(echo ';')$(NIX_QTREMOBJ)",) \
		&& cmake --build . --target yolo_ui -j$$(nproc)
	@echo "yolo_ui plugin ready at: $(BUILD_UI_PLUGIN)/libyolo_ui.so"

## Install yolo_ui plugin to logos-app plugins dir
install: build-ui-plugin
	mkdir -p ~/.local/share/Logos/LogosAppNix/plugins/yolo_ui
	cp $(BUILD_UI_PLUGIN)/libyolo_ui.so ~/.local/share/Logos/LogosAppNix/plugins/yolo_ui/yolo_ui.so
	@echo "yolo_ui installed to ~/.local/share/Logos/LogosAppNix/plugins/yolo_ui/yolo_ui.so"

## Install everything: yolo_ui + yolo
install-all: install install-module
	@echo ""
